...
async function safeFetch(path, opts = {}) {
  ...
  if (!text.trim()) {
    if (!res.ok) throw new Error(`Servidor retornou HTTP ${res.status} sem corpo.`);
    return {};
  }
  try {
...
function StatCard({ label, value, color = 'text-aw-purple' }) {
  return (
    ...
    <div className="aw-card p-4 space-y-2 border-aw-purple/30 bg-aw-purple/5">
      <div className="flex items-center justify-between text-sm">
        <span className="font-semibold text-aw-text flex items-center gap-2">
          ...
          {job.job_type}{' '}
          {job.is_dry_run && <span className="text-xs text-yellow-400 bg-yellow-500/20 px-2 py-0.5 rounded-full">DRY RUN</span>}
        </span>
...
export default function AdminImporter() {
  ...
  const [startingMsg,    setStartingMsg]    = useState('');
  const pollRef = useRef(null);

  const loadStats = useCallback(async () => {
    try {
      const [d, db] = await Promise.all([APIS.anfire.stats(), APIS.anibunker.stats()]);
      setStats(d);
  ...
  const loadJobs = useCallback(async (prov) => {
    setLoadingJobs(true);
    try {
      const d = await (APIS[prov] || APIS.anfire).jobs(1);
      const list = d.data || [];
      setJobs(list);
      setActiveJob(list.find(j => j.status === 'running' || j.status === 'pending') || null);
  ...
  const loadMappings = useCallback(async () => {
    setLoadingMap(true);
    try {
      const d = await (APIS[provider] || APIS.anfire).mappings(mappingPage, mapStatus, mapSearch);
      setMappings(d.data || []);
  ...
  const loadJobLogs = useCallback(async (jobId) => {
    try {
      const d = await (APIS[provider] || APIS.anfire).jobDetail(jobId);
  ...
  useEffect(() => {
    setJobs([]); setActiveJob(null); setSelectedJob(null); setJobLogs([]);
    loadStats();
    loadJobs(provider);
    clearInterval(pollRef.current);
    pollRef.current = setInterval(() => { loadStats(); loadJobs(provider); }, 5000);
    return () => clearInterval(pollRef.current);
  }, [provider]); // eslint-disable-line

  useEffect(() => { if (tab === 'mappings') loadMappings(); }, [tab, loadMappings]);

  // ── Inicia job com wake+retry real ────────────────────────
  const startJob = async (jobType, dryRun = false) => {
    if (activeJob) { toast.error('Já existe um job em execução.'); return; }
    setStarting(true);
    setStartingMsg('Conectando ao servidor...');
    const toastId = toast.loading('Conectando ao servidor...', { duration: 60000 });
    try {
      // 1) Garante servidor acordado (retry até 60s)
      await ensureServerAwake((msg) => {
        setStartingMsg(msg);
        toast.loading(msg, { id: toastId });
      });

      // 2) Dispara o job
      setStartingMsg('Iniciando job...');
      toast.loading('Iniciando job...', { id: toastId });
      ...
      toast.success(r.message || 'Job iniciado!', { id: toastId });
      await loadJobs(provider);
      await loadStats();
    } catch (e) {
      const msg = e.message || 'Erro desconhecido';
      toast.error(msg.substring(0, 120), { id: toastId });
    } finally {
      setStarting(false);
      setStartingMsg('');
    }
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { loadStats(); loadJobs(provider); }}
      ...
      {provider === 'anfire' && c && (
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-7 gap-3">
          <StatCard label="Total"          value={c.total}         color="text-aw-purple"  />
      ...
      {provider === 'anibunker' && ab && (
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-6 gap-3">
          <StatCard label="Total"          value={ab.total}              color="text-aw-purple"  />
      ...
      <div className="aw-card p-5">
        <div className="flex items-center justify-between mb-4">
          ...
          {starting && startingMsg && (
            <span className="text-xs text-aw-purple animate-pulse">{startingMsg}</span>
          )}
        </div>
        ...
        {activeJob && (
          <div className="flex gap-2 mt-4">
            <button onClick={() => handleCancel(activeJob.id)}
              ...
              <RiStopLine size={13} /> Cancelar job
            </button>
          </div>
        )}
      </div>
      ...
      <div className="flex gap-2 border-b border-aw-border pb-3">
        {[
          { v: 'overview', l: 'Jobs' },
          { v: 'mappings', l: `Mapeamentos${c?.review > 0 ? ` (${c.review})` : ''}` },
          { v: 'logs',     l: 'Logs' },
        ].map(t => (
        ...
        <div className="space-y-4">
          ...
          <div className="aw-card overflow-x-auto">
            <table className="w-full text-sm min-w-[700px]">
              <thead>
                <tr className="border-b border-aw-border">
                  {['Anime','Correspondência','Score','Status','Eps','Ações'].map(h => (
                    <th key={h} className="text-left px-3 py-3 text-xs font-semibold text-aw-dim uppercase tracking-wider">{h}</th>
                  ))}
                </tr>
              </thead>
                  ...
                  : mappings.map(m => (
                      <tr key={m.id} className={`border-b border-aw-border/50 hover:bg-white/2 ${m.status === 'review_required' ? 'bg-orange-500/5' : ''}`}>
                        <td className="px-3 py-3">
                          ...
                          <p className="text-xs text-aw-muted">eps: {m.episodes_count || 0}</p>
                        </td>
                        <td className="px-3 py-3">
                          ...
                          <div className="flex gap-1">
                            {m.status === 'review_required' && (
                              <button onClick={async () => {
                                ...
                                toast.success('Aprovado.'); loadMappings();
                              }} className="text-xs bg-green-500/20 text-green-400 hover:bg-green-500/30 px-2 py-1 rounded" title="Aprovar">
                                <RiCheckLine size={12} />
                              </button>
                            )}
                            <button onClick={async () => {
                              ...
                              toast.success('Removido.'); loadMappings();
                            }} className="text-xs bg-red-500/20 text-red-400 hover:bg-red-500/30 px-2 py-1 rounded" title="Remover">
      ...
      {tab === 'logs' && (
        <div className="space-y-4">
          <select className="aw-input w-full py-2 text-sm"
            onChange={e => e.target.value && loadJobLogs(e.target.value)} defaultValue="">
            ...
            <>
              ...
              <div className="aw-card p-0 overflow-hidden">
                <div className="max-h-[500px] overflow-y-auto p-3 space-y-1 font-mono text-xs">
                  {jobLogs.length === 0 && <p className="text-aw-muted text-center py-4">Nenhum log.</p>}
                  {jobLogs.map((l, i) => (
          ...
          {!selectedJob && (
            <div className="text-center py-16 text-aw-muted text-sm">
              Selecione um job acima para ver os logs.
            </div>
          )}
        </div>
      )}

      {/* Instruções AnFireAPI */}
      <details className="aw-card p-4">
        ...
        <div className="mt-4 space-y-3 text-sm text-aw-muted">
          ...
          <pre className="bg-aw-bg rounded-lg p-3 text-xs overflow-x-auto text-aw-purple">{`git clone https://github.com/MestreTM/AnFireAPI-Anime-Player\ncd AnFireAPI-Anime-Player/NodeJS\nnpm install`}</pre>
          <p><strong className="text-aw-text">2. Configure o .env e inicie:</strong></p>
          <pre className="bg-aw-bg rounded-lg p-3 text-xs overflow-x-auto text-aw-purple">{`API_KEY=sua_chave_aqui\nPORT=4000\nnode app.js`}</pre>
          <p><strong className="text-aw-text">3. Configure o backend (.env):</strong></p>
          <pre className="bg-aw-bg rounded-lg p-3 text-xs overflow-x-auto text-aw-purple">{`ANFIRE_API_URL=http://localhost:4000\nANFIRE_API_KEY=sua_chave_aqui`}</pre>
          <p className="text-yellow-400 flex items-center gap-1.5 mt-3">
            <RiAlertLine size={14} /> O Anibunker funciona sem configuração extra — use direto.
          </p>
        </div>
      </details>
    </div>
