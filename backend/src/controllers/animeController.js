...
async function getAnimes(req, res, next) {
  try {
    const {
      ...
      type, sort = 'popularity', order = 'desc', q,
    } = req.query;
    ...
    if (genre) {
      ...
      conditions.push(`EXISTS (
        ...
        WHERE ag.anime_id = a.id AND g.slug = $${params.length}
      )`);
    }
    ...
    const countResult = await query(
      `SELECT COUNT(*) FROM anime a ${whereClause}`,
      params
    );
    ...
    const result = await query(
      `SELECT a.id, a.slug, a.title, a.title_english, a.cover_url, a.banner_url,
              a.year, a.season, a.status, a.type, a.episodes_count,
              a.score, a.popularity, a.studio,
              COALESCE(
  ...
  try {
    const result = await query(
      `SELECT a.id, a.slug, a.title, a.title_english, a.description,
              ...
              a.year, a.status, a.type, a.episodes_count, a.score,
              COALESCE(
       ...
       LEFT JOIN anime_genres ag ON ag.anime_id = a.id
       LEFT JOIN genres g ON g.id = ag.genre_id
       WHERE a.banner_url IS NOT NULL AND a.is_hidden = false
       GROUP BY a.id
  ...
  try {
    ...
    const result = await query(
      `SELECT a.id, a.slug, a.title, a.title_english, a.cover_url,
              a.year, a.type, a.score, a.episodes_count, a.status,
              COALESCE(
                json_agg(DISTINCT jsonb_build_object('name', g.name, 'slug', g.slug))
                FILTER (WHERE g.id IS NOT NULL), '[]'
              ) AS genres
...
async function getBySeason(req, res, next) {
  try {
    ...
    const result = await query(
      `SELECT a.id, a.slug, a.title, a.title_english, a.cover_url,
              a.year, a.season, a.type, a.score, a.status, a.episodes_count,
              COALESCE(
                json_agg(DISTINCT jsonb_build_object('name', g.name, 'slug', g.slug))
                FILTER (WHERE g.id IS NOT NULL), '[]'
              ) AS genres
